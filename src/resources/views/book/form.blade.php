...
<form method="post" enctype="multipart/form-data" action="{{ $book->exists ? '/books/patch/' . $book->id : '/books/put' }}">
    @csrf
    ...
    <div class="mb-3">
        ...
        <select id="book-author" name="author_id" class="form-select @error('author_id') is-invalid @enderror">
            ...
            <option value="{{ $author->id }}" @if ($author->id == old('author_id', $book->author->id ?? false)) 
                selected @endif>{{ $author->name }}</option>
            @endforeach
        </select>
        @error('author_id')
